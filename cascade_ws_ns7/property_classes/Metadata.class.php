...
/**
  ...
  * 1/23/2019 Added getMetaDataValues.
...
/**
<documentation><description><p>Returns an array containing names of wired fields
and dynamic field as keys and field values as values. The value
associated with a dynamic field name can be either an array of strings or NULL. This
array can be fed directly into <code>a\DublinAwareAsset::updateMetadata()</code> .</p></description>
<example>u\DebugUtility::dump( $page->getMetadata()->getMetaDataValues() );</example>
<return-type>array</return-type>
</documentation>
*/
    public function getMetaDataValues() : array
    {
        ...
        foreach( a\DublinAwareAsset::$wired_fields as $wired_field )
        {
            $method_name = "get" . ucwords( $wired_field );
            $values[ $wired_field ] = $this->$method_name();
        }
        
        foreach( $this->getDynamicFieldNames() as $dynamic_field )
        {
            $dynamic_values = $this->getDynamicFieldValues( $dynamic_field );
            
            if( count( $dynamic_values ) == 1 && is_null( $dynamic_values[ 0] ) )
            {
                $dynamic_values = NULL;
            }
        
            $values[ $dynamic_field ] = $dynamic_values;
        }
